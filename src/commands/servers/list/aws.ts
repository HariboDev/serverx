...
export default class ListCommandAWS extends Command {
  static description = `Display AWS servers
...
`;
  ...
  async run(): Promise<void> {
    const { flags }: any = await this.parse(ListCommandAWS);

    const instancesData: IInstancesData | undefined = await getAWS(flags, this.config);
    ...
    for (const instance of instances.aws) {
      let stateChalk;
      ...
      table.push([
        instances.aws.indexOf(instance),
