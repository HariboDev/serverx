...
export default async function getGCP(flags: FlagInput<any>, config: Config): Promise<any> {
  ...
  flags.region.toString() === "all" ? instancesData.gcp = await listAllInstances("new-project-12365") : instancesData.gcp = await listInstanceByZone("new-project-12365", flags.region.toString().split(","));
